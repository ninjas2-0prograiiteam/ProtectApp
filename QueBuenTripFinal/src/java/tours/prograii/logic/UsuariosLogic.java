...
public class UsuariosLogic extends Logic {
    ...
    {
        DatabaseX database = getDatabase();
        String strSql = "SELECT * from miseventosusuario where username = '"+p_strUsername+"'";
        System.out.println(strSql);
        ResultSet CResult = database.executeQuery(strSql);
            ...
            try 
            {
                while(CResult.next())
                {
                   ...
                   strDepartamento = CResult.getString("departamento");
                    ...
                    CTemp = new EventoViewObj(strUsername, strCategoria, strDepartamento, strHoradeSalida, strFecha, strHoradeRetorno, strPuntodeEncuentro, iPrecio,iCuposRestantes);
                    CArray.add(CTemp);
                }
            } 
            catch (SQLException ex) 
